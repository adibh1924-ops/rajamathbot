...
CHARTS_DATA = {
    'chart_1402': {"title": "چارت درسی ورودی ۱۴۰۲ آموزش ریاضی", "file_id": "YOUR_FILE_ID_1402"},
    'chart_1403': {"title": "چارت درسی ورودی ۱۴۰۳ آموزش ریاضی", "file_id": "YOUR_FILE_ID_1403"},
    'chart_1404': {"title": "چارت درسی ورودی ۱۴۰۴ آموزش ریاضی", "file_id": "YOUR_FILE_ID_1404"},
}
...
VIDEOS_DATA = {
    'vid_math1': {
        ...
        "items": [
            ...
            {"name": "جلسه دوم: مشتق و کاربردها", "link": "https://t.me/c/xxxx/2"}
        ]
    },
    'vid_math2': {
        "title": "ریاضی عمومی ۲",
        "items": [
            {"name": "جلسه اول: بردارها و هندسه تحلیلی", "link": "https://t.me/c/xxxx/3"}
        ]
    },
    'vid_eq': {
        "title": "معادلات دیفرانسیل",
        "items": [
            {"name": "جلسه اول: معادلات مرتبه اول", "link": "https://t.me/c/xxxx/4"}
        ]
    },
    'vid_eng': {
...
def get_join_channel_menu():
    ...
    markup.add(InlineKeyboardButton("📢 عضويت در کانال رسمی", url=f"https://t.me/{CHANNEL_USERNAME.replace('@', '')}"))
    markup.add(InlineKeyboardButton("🔄 بررسی دوباره عضویت", callback_data='check_sub_again'))
    return markup
...
def get_main_menu():
    ...
    markup.add(
        ...
        InlineKeyboardButton("📄 جزوات و منابع ریاضی", callback_data='menu_handouts'),
        ...
        InlineKeyboardButton("📞 پشتیبانی و ارسال فایل", callback_data='menu_support')
    )
    return markup
...
def send_welcome(message):
    ...
    if not check_subscription(user_id):
        warning_text = (
            ...
            "❌ برای استفاده از امکانات ربات، لطفاً ابتدا در کانال ما عضو شوید و سپس روی دکمه «بررسی دوباره عضویت» کلیک کنید:"
        )
        bot.send_message(message.chat.id, warning_text, reply_markup=get_join_channel_menu())
        return
    ...
    if call.data == 'check_sub_again':
        if check_subscription(user_id):
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, 
                                  text="✨ خوش آمدید! لطفاً از منوی زیر استفاده کنید:", reply_markup=get_main_menu())
        else:
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, 
                              ...
                              parse_mode="Markdown", reply_markup=get_chart_menu())

    elif call.data == 'menu_videos':
        bot.answer_callback_query(call.id)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, 
                              text="🎬 **بخش ویدیوهای آموزشی**\n\nدرس مورد نظر را انتخاب کنید:", 
                              parse_mode="Markdown", reply_markup=get_videos_menu())

    elif call.data == 'menu_handouts':
        bot.answer_callback_query(call.id)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, 
                              text="📄 **بانک جزوات و منابع ریاضی**\n\nدرس مورد نظر خود را انتخاب کنید:", 
                              parse_mode="Markdown", reply_markup=get_handouts_menu())

    elif call.data == 'menu_podcasts':
        bot.answer_callback_query(call.id)
        text = "🎙 **نشریات و پادکست‌های انجمن علمی ریاضی**\n\nلیست شماره‌های نشریه و پادکست‌های صوتی در این بخش قرار دارد."
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=text, parse_mode="Markdown", reply_markup=get_back_menu())

    elif call.data == 'menu_links':
        bot.answer_callback_query(call.id)
        text = "🔗 **لینک‌های مفید و کاربردی:**\n\n🔹 سامانه گلستان\n🔹 کانال تلگرامی انجمن\n🔹 سایت دانشکده"
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=text, parse_mode="Markdown", reply_markup=get_back_menu())

    elif call.data == 'menu_support':
        bot.answer_callback_query(call.id)
        text = "📞 **ارتباط با پشتیبانی و ارسال فایل**\n\nپیام یا فایل خود را ارسال کنید تا به دست ادمین برسد:"
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=text, parse_mode="Markdown", reply_markup=get_back_menu())
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, 
                              text="✨ منوی اصلی ربات:", reply_markup=get_main_menu())

    elif call.data in CHARTS_DATA:
        bot.answer_callback_query(call.id)
        item = CHARTS_DATA[call.data]
        bot.send_photo(call.message.chat.id, item['file_id'], caption=f"📄 {item['title']}")
        ...
        bot.send_message(call.message.chat.id, f"📁 منابع و جزوات مربوط به درس **{course['title']}**:")
        for file_info in course['files']:
            if file_info['file_id'].startswith('FILE_ID_'):
                bot.send_message(call.message.chat.id, f"🔸 {file_info['name']} (فایل هنوز تنظیم نشده است)")
            else:
                bot.send_document(call.message.chat.id, file_info['file_id'], caption=file_info['name'])
...
def receive_user_file_or_message(message):
    ...
    try:
        ...
        bot.reply_to(message, "✅ پیام و فایل شما با موفقیت به دست ادمین رسید. سپاس! 🌹")
    except Exception as e:
